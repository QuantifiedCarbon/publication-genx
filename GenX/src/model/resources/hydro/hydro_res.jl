...
function hydro_res!(EP::Model, inputs::Dict, setup::Dict)
	...
	# commenting this one out to not have duplicate constraints
	...
	# Add constraints for the minimum energy in storage for hydro resources. I have a file from joinpath(path, hydro_min_level.csv) that has the minimum energy level for each hydro resource for each time step. This level is a fraction of the total energy storage capacity. 
	path = inputs["path"]
	# read in the minimum energy level for each hydro resource for each time step to a DataFrame like 	gen_in = DataFrame(CSV.File(joinpath(path, filename), header=true), copycols=true)
	min_level = DataFrame(CSV.File(joinpath(path, "hydro_min_level.csv"), header=true), copycols=true)

	# print the first few rows of min_level
	println("First few rows of min_level")
	show(min_level, allcols=true)

	# create dictionary to map HYDRO_RES to column names in min_level
	dict = Dict(zip(HYDRO_RES, names(min_level)))
	print("dict: ", dict)
	
	# print the average monthly min level for each hydro resource
	println("Average min level for each hydro resource")
	for y in HYDRO_RES
		println(y, " ", mean(min_level[!,dict[y]]))
	end

	# print the 31st hour min level for each hydro resource
	println("31st hour min level for each hydro resource")
	for y in HYDRO_RES
		println(y, " ", min_level[31,dict[y]])
	end

	# print min_level[t, dict[y]]+0.0)*dfGen[y,:Hydro_Energy_to_Power_Ratio]*EP[:eTotalCap][y] for each hydro resource and time step 0
	println("min_level[t, dict[y]]+0.0)*dfGen[y,:Hydro_Energy_to_Power_Ratio]*EP[:eTotalCap][y] for each hydro resource and time step 0")
	for y in HYDRO_RES
		println(y, dict[y], " ", (min_level[1, dict[y]]+0.0)*dfGen[y,:Hydro_Energy_to_Power_Ratio]*EP[:eTotalCap][y])
	end

	# add constraints for the minimum energy level for each hydro resource for each time step, recall that the minimum energy level is a fraction of the total energy storage capacity
	# use the dict to map HYDRO_RES to column names in min_level
	@constraint(EP, cHydroMinEnergy[y in HYDRO_RES, t in 1:T], EP[:vS_HYDRO][y,t] >= (min_level[t, dict[y]]+0.0)*dfGen[y,:Hydro_Energy_to_Power_Ratio]*EP[:eTotalCap][y])
	
	# do the same for maximum energy level for hydro resources
	max_level = DataFrame(CSV.File(joinpath(path, "hydro_max_level.csv"), header=true), copycols=true)
	dict = Dict(zip(HYDRO_RES, names(max_level)))

	@constraint(EP, cHydroMaxEnergy[y in HYDRO_RES, t in 1:T], EP[:vS_HYDRO][y,t] <= (max_level[t, dict[y]]+0.0)*dfGen[y,:Hydro_Energy_to_Power_Ratio]*EP[:eTotalCap][y])
