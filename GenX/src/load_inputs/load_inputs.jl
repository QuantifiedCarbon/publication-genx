...
function load_inputs(setup::Dict,path::AbstractString)
	...
	# Read in transmission availability profiles
    if isfile(joinpath(path,"Transmission_variability.csv"))
		inputs = load_transmission_variability(setup, path, inputs)
	elseif setup["Transmission_variability"] == 1
		throw(error("Transmission_variability constraint is active but no input file 'Transmission_variability.csv' exists"))
	end
